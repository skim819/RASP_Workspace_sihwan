	...
	for line in f:
		...
		if len(words):
			if words[0] == '.subckt':
				...
				if subckt in ['lpf_2','meas_volt_mite','cap','ota_buf','ota_buffer','lpf','nfet_i2v','pfet_i2v','i2v_pfet_gatefgota','nmirror','nmirror_vmm','TIA_blk','fgswc_nmirror_w_bias','bias_gen']: ## single i/ps
					...
					key=ports[3]
				elif subckt in ['gnd_out','vdd_out','nmirror_w_bias','ichar_nfet','ramp_fe']: #2 i/ps
					key=ports[2]
